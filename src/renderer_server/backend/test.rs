...
use crate::ipc_protocol::{connect_client, connect_server, ClientReceiver, ClientSender, ConnectionError};

use super::super::{app::SharedApp, renderer::display_list::SharedDisplayList, serve, test_event_loop_notifier::EventLoopNotifier};
...
impl RendererServer {
    ...
    pub async fn spawn() -> Result<(Self, ClientSender, ClientReceiver), ConnectionError> {
        ...
        let (child, task_handle) = async move {
            let server_name = server_name_receiver.await.expect("bug: unable to receive server name");
...
pub async fn run_main(server_name: String) {
    ...
    let (conn_sender, conn_receiver) = connect_server(server_name).expect("unable to establish turtle server connection");
