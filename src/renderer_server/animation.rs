...
impl MoveAnimation {
    pub fn new(turtle: &mut TurtleDrawings, display_list: &mut DisplayList, target_pos: Point) -> Self {
        let TurtleState {
            position, speed, ref pen, ..
        } = turtle.state;
    ...
    pub fn write_current_state(&self, state: &mut TurtleState, current_fill_polygon: Option<PrimHandle>, display_list: &mut DisplayList) {
...
impl RotateAnimation {
    pub fn new(turtle: &mut TurtleDrawings, delta_angle: Radians, direction: RotationDirection) -> Self {
...
impl AnimationRunner {
    pub fn new(conn: ServerSender, app: SharedApp, display_list: SharedDisplayList, event_loop: EventLoopNotifier) -> Self {
        let (sender, receiver) = mpsc::unbounded_channel();

        tokio::spawn(animation_loop(conn, app, display_list, event_loop, receiver));
...
fn compute_next_update(next_frame: time::Instant, animations: &HashMap<TurtleId, Animation>) -> time::Instant {
    let next_update = animations.values().map(|anim| anim.next_update()).min().unwrap_or(next_frame);
