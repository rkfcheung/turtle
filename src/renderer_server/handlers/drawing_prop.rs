...
pub(crate) fn drawing_prop(conn: ServerOneshotSender, app: &App, prop: DrawingProp) -> Result<(), HandlerError> {
...
pub(crate) fn set_drawing_prop(app: &mut App, event_loop: &EventLoopNotifier, prop_value: DrawingPropValue) -> Result<(), HandlerError> {
    let mut drawing = app.drawing_mut();

    modify_drawing(&mut drawing, event_loop, prop_value)
}

pub(crate) fn reset_drawing_prop(app: &mut App, event_loop: &EventLoopNotifier, prop: DrawingProp) -> Result<(), HandlerError> {
    let mut drawing = app.drawing_mut();
...
fn modify_drawing(drawing: &mut DrawingState, event_loop: &EventLoopNotifier, prop_value: DrawingPropValue) -> Result<(), HandlerError> {
