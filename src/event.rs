    ...
    pub(crate) fn from_window_event(
        ...
        match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                ..
            } => Some(Event::Key(Key::from_keycode(virtual_keycode?)?, PressedState::from_state(state))),
            WindowEvent::CursorEntered { .. } => Some(Event::WindowCursor(true)),
            WindowEvent::CursorLeft { .. } => Some(Event::WindowCursor(false)),
            WindowEvent::CursorMoved { position, .. } => Some(Event::MouseMove(to_logical(position))),
...
impl Key {
    ...
    fn from_keycode(key: glutin_event::VirtualKeyCode) -> Option<Self> {
        ...
        Some(match key {
            ...
            Snapshot | Scroll | Pause | Insert | Compose | Caret | Numlock | AbntC1 | AbntC2 | Apps | Ax | Calculator | Capital | Convert
            | Kana | Kanji | LAlt | LControl | LShift | LWin | Mail | MediaSelect | MediaStop | Mute | MyComputer | NavigateForward
            | NavigateBackward | NextTrack | NoConvert | OEM102 | PlayPause | Power | PrevTrack | RAlt | RControl | RShift | RWin | Sleep
            | Stop | Sysrq | Underline | Unlabeled | VolumeDown | VolumeUp | Wake | WebBack | WebFavorites | WebForward | WebHome
            | WebRefresh | WebSearch | WebStop | Yen | Copy | Paste | Cut => return None,
        })
    }
}
